...
function BoardCard({ board, onDelete, onDuplicate, onShare, viewMode }) {
  ...
  return (
    ...
    >
      {/* Thumbnail */}
      <div className="h-36 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-700 dark:to-gray-750 relative overflow-hidden flex items-center justify-center">
        {board.thumbnail_url ? (
          <img
        ...
        >
          <span
            className="opacity-0 group-hover:opacity-100 transition-opacity
                           text-white text-sm font-semibold bg-black/50 px-3 py-1.5 rounded-lg"
          >
            Open Board
          </span>
        </div>
...
/* ─────────────────────────────────────────────────────────────
   Dashboard
───────────────────────────────────────────────────────────── */
export default function Dashboard({ showLibrary = false }) {
  const navigate = useNavigate();
  const { user } = useAuthStore();
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
            {showLibrary ? (
              <>
                <BookOpen className="inline w-6 h-6 mr-2 text-primary-500" />
                Library
              </>
            ) : (
              "My Boards"
        ...
        <div className="mb-6 p-4 rounded-2xl bg-gradient-to-r from-primary-50 to-purple-50 dark:from-primary-900/20 dark:to-purple-900/20 border border-primary-200 dark:border-primary-700 flex items-center justify-between gap-4">
          <div>
            <p className="text-sm font-semibold text-primary-700 dark:text-primary-300">
              Board limit reached ({limit}/{limit})
            </p>
            <p className="text-xs text-primary-600/70 dark:text-primary-400/70 mt-0.5">
              Upgrade your plan to create unlimited boards.
            </p>
          </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-64 text-center">
          ...
          {!search && (
            <Button icon={Plus} size="lg" onClick={() => setNewBoardOpen(true)}>
