...
const PLAN_PRICES = { lite: 0, pro: 15000, premium: 30000 };
  ...
  try {
    ...
    await query(
      `INSERT INTO subscriptions (id, user_id, plan, status, order_id, amount, payment_method, created_at)
       VALUES ($1, $2, $3, 'pending', $4, $5, 'qris', NOW())`,
      [subId, req.user.id, plan, orderId, amount],
    );
    ...
    await query(
      `INSERT INTO payments (id, user_id, subscription_id, order_id, amount, fee, total_payment, payment_method, payment_number, status, expired_at, pakasir_data, created_at)
       VALUES (uuid_generate_v4(), $1, $2, $3, $4, $5, $6, 'qris', $7, 'pending', $8, $9, NOW())`,
      [
        req.user.id,
        subId,
        orderId,
    ...
    const paymentUrl = `${PAKASIR_BASE_URL}/pay/${PAKASIR_PROJECT}/${amount}?order_id=${orderId}&qris_only=1&redirect=${process.env.FRONTEND_URL}/payment/success`;
